...
=======
>>>>>>> 321b28747683208445e70b9de2a4f75d74850f47
...
- .NET 10 (ASP.NET Core 10) â€” server and SignalR hub, Razor Pages for server routing/hosting.
- SignalR â€” real-time bidirectional communication between server and clients.
- React 19.2 â€” client UI and hub consumer.
- Tailwind-style utility classes used in the React app for styling.
- Docker â€” container images and Dockerfile included for producing reproducible images.
- JSON â€” message format for payloads sent over SignalR.
- Asynchronous programming patterns (async/await) â€” used throughout to maximize scalability and responsiveness.

## Project layout (high level)

- `SportsTicker/` â€” ASP.NET Core 10 project (Razor Pages) hosting the SignalR hub and API endpoints.
- `sports-ticker-app/` â€” React 19.2 front-end application that connects to the SignalR hub. Uses `VITE_SIGNALR_HUB_URL` via `import.meta.env` to locate the server.
- `Dockerfile` â€” multi-stage Dockerfile to build and publish the server image.
